...
export class RoomManager {
   ...
   * @param {boolean} [options.enabled] - Enable backpressure-aware broadcasting.
   * @param {number} [options.highWaterMark] - Bytes threshold to flag a client as slow.
   * @param {number} [options.slowConsumerTimeout] - Ms before terminating a slow consumer.
   * @param {number} [options.batchSize] - Sends per event-loop tick in backpressure mode.
   */
  constructor({
    maxRoomSize,
    ...
    this._backpressureOptions = enabled
      ? { enabled: true, highWaterMark, slowConsumerTimeout, batchSize }
      : { enabled: false };
